...
{
    public class MoveLightMutation : IMutation
    {
        ...
        private static string name_ID = "Light Move Spawn Mutation";

        public MoveLightMutation(double mutation_rate)
        {
        ...
        public void mutate(IGenotype geno)
        {
            try
            {
                ...
                int pos = getRandomLightPos(gen);

                // If there are lights, move them around. Else just don't do anything.
                if (pos != -1)
                {
                    ...
                    if (logger != null)
                    {
                        logger.writeLog("Light Move Mutation -- Moved Light from Room " + chosen_spawn[1] + " to Room " + new_room);
                    }
        ...
        private int getRandomLightPos(Genotype gen)
        {
            List<int> pos = new List<int>();

            for (int i = 0; i < gen.getSpawnPoints().Count; i++)
            {
                if (gen.getSpawnPoints()[i][0] == 4) // THIS CHANGE!
                {
                    pos.Add(i);
                }
            }
