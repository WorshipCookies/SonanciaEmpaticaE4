...
//public 
	...
	//File Options
	public string searchPattern = "*.esc"; //search pattern used to find files
	//Output
...
//Protected	
	...
	//Constructors
	public FileBrowser(string directory,int layoutStyle,Rect guiRect){	currentDirectory = new DirectoryInfo(directory + "\\ExperimentFiles");	layout = layoutStyle;	guiSize = guiRect;	}
						...
						if(isSearching){
							...
							for(int fi=0;fi<files.Length;fi++){
								...
								if(files[fi].button()){
									...
									selectedFile = fi;
                                    Debug.Log("Output File is = " + outputFile.ToString());
                                }
								if(selectedFile==fi)
				...
				if(isSearching){
					...
					for(int fi=0;fi<files.Length;fi++){
						...
						if(files[fi].button()){
							outputFile = files[fi].fi;
							selectedFile = fi;
                            Debug.Log("Output 2 File is = " + outputFile);
                        }
						if(selectedFile==fi)
							GUI.color = defaultColor;
					}
